...
export function registerIpcHandlers(appService: AppService) {
  ...
  ipcMain.handle(IPC_CHANNELS.workspacesScanArtifacts, async (_event, taskId: string) => {
    try {
      const records = appService.listTaskArtifacts(taskId)
      const resultArtifacts: WorkspaceArtifact[] = []

      // 仅返回已显式关联到当前任务的产物，不能按整个工作区兜底扫描。
      for (const rec of records) {
        try {
          const stat = await fs.stat(rec.absolutePath)
      ...
      // 按更新时间倒序排列 (最新的产物置顶)
      resultArtifacts.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())
      return ok(resultArtifacts)
    } catch (error) {
      return fail(error)
    }
  })
