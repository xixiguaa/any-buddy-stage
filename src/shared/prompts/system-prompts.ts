...
【工具说明】可用的内置工具包括：ls（列出目录）、read_file（读取文件）、write_file、edit_file、grep（在工作区内搜索文本）、glob（按模式匹配文件名）、execute（执行命令）、task（调度子 Agent 协作）。read_write 权限下，execute 在全局共享的本地 Docker 沙箱中执行并先等待用户确认；full_access 权限下，execute 通过本地 Shell backend 直接执行。此外项目挂载的 web_search 用于在设置中开启 Web 搜索后调用。
