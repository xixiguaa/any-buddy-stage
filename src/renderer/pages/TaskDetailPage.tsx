...
export default function TaskDetailPage() {
  ...
  const timelineCards = useMemo(
    () => taskEvents.slice().reverse().slice(0, 16).map(buildRuntimeEventCard),
    [taskEvents],
  );
  ...
  const activeSubagentRun = useMemo(
    () => subagentRuns.find(run => run.id === activeSubagentId) ?? null,
    [activeSubagentId, subagentRuns],
  );

  const activeSubagentMessages = useMemo(
    () => (activeSubagentRun ? getRunThreadMessages(messages, activeSubagentRun) : []),
    [activeSubagentRun, messages],
  );
  ...
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', background: '#ffffff', width: '100%' }}>
      ...
      <div style={{ width: '420px', borderLeft: '1px solid #f1f5f9', background: '#fcfcfd', padding: '20px 16px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '14px', padding: '14px' }}>
          <div style={{ fontSize: '13px', fontWeight: 700, color: '#0f172a', marginBottom: '10px' }}>工具调用</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {toolCards.map(card => (
              <div key={card.id} style={{ border: '1px solid #e2e8f0', borderRadius: '12px', padding: '10px 12px', background: '#f8fafc' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a' }}>{card.title}</div>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>{card.description}</div>
                  </div>
                  <Tag color={card.tone === 'warning' ? 'warning' : card.tone === 'error' ? 'error' : card.tone === 'success' ? 'success' : 'processing'}>
                    {card.status ?? 'running'}
                  </Tag>
                </div>
                {card.detail && (
                  <pre style={{ margin: '8px 0 0', whiteSpace: 'pre-wrap', fontSize: '11px', color: '#334155', background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '8px', overflow: 'auto' }}>
                    {card.detail}
                  </pre>
                )}
              </div>
            ))}
            {toolCards.length === 0 && <div style={{ fontSize: '12px', color: '#94a3b8' }}>当前没有工具调用卡。</div>}
          </div>
        </div>
              ...
              >
                ...
                {run.kind === 'subagent' && (
                  <div style={{ fontSize: '11px', color: '#64748b', marginTop: '8px' }}>
                    线程消息数 {getRunThreadMessages(messages, run).length}
                  </div>
                )}
              </div>
            ))}
          ...
          {activeSubagentRun && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                {activeSubagentMessages.map(message => {
                  ...
                  return (
                    <div key={message.id} style={{ border: '1px solid #e2e8f0', borderRadius: '10px', padding: '10px', background: isUser ? '#eff6ff' : '#f8fafc' }}>
                      ...
                      <div style={{ fontSize: '12px', color: '#0f172a', lineHeight: 1.6, whiteSpace: 'pre-wrap', fontFamily: isTool ? 'Consolas, Courier New, monospace' : 'inherit' }}>
                        {message.content}
                      </div>
                    </div>
                  );
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <Input.TextArea
                  rows={4}
                  value={subagentMessageText}
                  onChange={event => setSubagentMessageText(event.target.value)}
                  placeholder="补充上下文、调整策略，或者要求子 Agent 继续追查。"
                />
