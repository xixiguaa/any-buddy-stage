...
import { getPlanApprovalText, isPlanApproval } from './task-detail-utils.js'
...
export function TaskDetailProvider({ children }: { children: ReactNode }) {
  ...
  // 防止较早的扫描请求在较晚完成时覆盖最新产物列表。
  const artifactScanRequestIdRef = useRef(0)
  ...
  const scanArtifacts = async (): Promise<WorkspaceArtifact[]> => {
    if (!taskId) return []
    const requestId = ++artifactScanRequestIdRef.current
    setIsScanningArtifacts(true)
    try {
      const res = await rendererApi.workspace.scanArtifacts(taskId)
      if (res.ok) {
        if (requestId === artifactScanRequestIdRef.current) {
  ...
  }, [taskId, isAgentWorking, task?.updatedAt])
